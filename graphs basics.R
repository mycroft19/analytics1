...
plot(women, type='l')
plot(women, type='b')
plot(women, type='b', pch=18, lty=8, col=2)
plot(women, xlim=c(30,100), ylim=c(min(women$weight)-10, 200), pch=10)

#more features with plot
plot(x=women$weight, y=women$height, pch=1, xlab='Weight', ylab='Height', col='purple', cex=2, type='b')
title(main='Main Title', sub='Sub Title')
#see cheat sheet on base graphs

plot(women)
abline(lm(women$weight ~ women$height), col='black', lty=2, lwd=4)
...
gender
table(gender)
pie(table(gender),labels = c('XY','XX'))
x = c(10,20,40,50)
pie(x)
...
x
qw=mtcars
a1=table(mtcars$cyl,mtcars$hp)
barplot(a1,1:4)
mtcars %>% barplot(col=1)
#barplot
barplot(x,col=1:4)
